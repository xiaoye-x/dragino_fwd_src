...
LoRaMacParserStatus_t LoRaMacParserJoinAccept( LoRaMacMessageJoinAccept_t* macMsg )
{
    ...
    //bufItr = bufItr + 3;
    macMsg->JoinNonce[0] = macMsg->Buffer[bufItr++];
    macMsg->JoinNonce[1] = macMsg->Buffer[bufItr++];
    macMsg->JoinNonce[2] = macMsg->Buffer[bufItr++];

    //lgw_memcpy( macMsg->NetID, &macMsg->Buffer[bufItr], 3 );
    //bufItr = bufItr + 3;
    macMsg->NetID[0] = macMsg->Buffer[bufItr++];
    macMsg->NetID[1] = macMsg->Buffer[bufItr++];
    macMsg->NetID[2] = macMsg->Buffer[bufItr++];

    macMsg->DevAddr =  macMsg->Buffer[bufItr++];
...
void decode_mac_pkt_down(LoRaMacMessageData_t* macMsg, void* pkt)
{
    ...
    switch (macMsg->MHDR.Bits.MType) {
        ...
        case FRAME_TYPE_JOIN_ACCEPT: 
            ...
            printf("#########DEBUG###########\n");
            int i;
            for (i = 0; i < joinMsg.BufSize; i++) {
                printf("%02X", joinMsg.Buffer[i]);
            }
            printf("\n#########DEBUG###########\n");
